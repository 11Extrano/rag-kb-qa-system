...
基于企业自有知识库的 **RAG（检索增强生成）AI 问答系统**。📄 上传知识库后自动清洗、分块、向量化入库；💬 用户提问后检索最相关片段，交由 🤖 大模型生成带引用来源的答案，✨ 前端 **SSE** 流式展示。同一**会话 id** 下支持**多轮追问**，与单轮请求兼容；原理与演示见**第四节「技术亮点」第 1 条**与**第五节「系统演示」**。

📐 项目采用 DDD 领域建模 + OpenSpec 规格驱动开发，每次变更都有设计文档和规格可追溯，架构边界清晰、迭代可控。**当前版本功能已按规格交付**，可直接本地启动体验（参见 [项目运行指南](docs/项目运行.md)）。
...
**OpenSpec（SDD 规格驱动开发框架）**：项目累计经过多轮变更，每轮都产出 Proposal → Design → Specs → Tasks 一套完整文档，全部归档可追溯：
...
| `support-context` | 多轮会话、检索前改写、生成侧历史与 token 裁剪 |
...
### 1. 多轮 RAG：对话驱动的检索改写 + 生成侧历史上下文

多轮场景里，**「搜到什么」**和**「怎么答」**必须拆开，否则要么向量检索对不齐知识库表述，要么回答接不上上一轮话术。

**检索侧——先对齐「可嵌入的问句」，再去做向量匹配**  
用户第二轮常说「第二步要准备什么？」「境外用哪个表？」——若把这句**原样**向量化，往往和文档里书面、完整的句子**语义距离偏大**，Top-K 容易偏题。存在会话历史时，系统先用**单独一次大模型调用**（专用指令，与「写答案」那次调用职责不同），把「历史 + 本轮输入」收成**一句独立、无指代歧义的中文检索问句**，再**只对这一句**做嵌入与向量检索并回表取原文。送入向量空间的是与知识库风格一致的完整问句，而不是口语碎片，**从机制上提高召回精度**；若该步失败，则回退用本轮用户原句检索，避免整条链路被拖死。

**生成侧——参考内容管事实，历史管语境**  
检索到的片段仍以【参考内容】约束**事实**（有据才答）；同时把经 **token 预算与裁剪**后的多轮对话写入提示，让模型理解「上一轮助手说了什么」，从而消解指代、承接语气、处理「再说一次」等追问。**上下文能力**来自「裁剪后的结构化历史 + 本轮参考块」，而非无差别堆砌整段聊天；流式与一次性返回共用同一套组装与裁剪策略。超长时优先收紧低相关参考、再成对收缩更早轮对话等，避免提示爆炸。

**小结**：**嵌入与检索针对「改写后的检索句」，负责搜得准；历史进入作答提示，负责答得连贯。** 与单轮「用户原句直接嵌入检索」相比，这是多轮 RAG 的核心差异。

### 2. 两段式分块策略（标题感知 + 固定长度 + Overlap）
...
### 5. 大模型集成：OpenAI 兼容协议 + Provider 抽象
...
以下演示需先向系统上传示例知识库中的 **`退款流程与申请表.md`**（及按需其他文档），待索引完成后再在**同一浏览器会话**内连续提问（与 [`demo-knowledge-base/README.md`](demo-knowledge-base/README.md)、《[多轮知识库对话-上下文设计说明](docs/多轮知识库对话-上下文设计说明.md)》中的退款多轮场景一致）。

### 演示问题（可照抄）

| 轮次 | 用户问题（示例） |
|------|------------------|
| 1 | 退款线上申请流程分几步？每一步大概做什么？回答简短一点 |
| 2 | 第二步要准备什么？回答简短一点 |
| 3 | 没看懂，再说一次 |

### 界面截图

![步骤1：首轮提问——退款线上申请流程分几步](docs/ask_step1.jpeg)

![步骤2：追问第二步要准备什么与参考来源](docs/ask_setp2.jpeg)

![步骤3：澄清「再说一次」](docs/ask_setp3.jpeg)

### 实现要点（对应上图）

| 轮次 | 现象 | 背后在做什么 |
|------|------|----------------|
| 1 | 助手按步骤概括退款流程 | 用户完整问句 → 嵌入检索 → 命中文档片段 → 在「仅依据参考内容」约束下生成回答。 |
| 2 | 短句「第二步…」仍能命中表格类文档，且展示**参考来源**与相似度 | 结合上一轮对话，先把追问**改写**成适合与知识库对齐的完整检索句，再嵌入与检索；作答仍只依据本轮检索到的片段，避免编造。 |
| 3 | 用更口语的方式复述第二步，贴近「没听懂再讲一遍」 | 在参考片段约束事实的前提下，把**多轮对话历史**纳入作答提示，换表述复述，而不是仅重复第一轮检索路径。 |
...
│   │   │   ├── conversation.ts      #   按会话 id 只读拉取消息（供编排/调试）
│   │   │   └── qa.ts                #   问答入口，多轮编排，SSE 流式分支，协议适配
│   │   │
...
│   │   │   │                        #   【检索匹配域】
│   │   │   ├── retrievalMatch.ts    #   领域核心：多轮时检索前改写 + 嵌入检索 + MySQL 关联
│   │   │   │                        #   【答案生成域】
...
│   │       ├── conversation.ts      #   会话
│   │       ├── conversation_message.ts # 消息（user/assistant）
│   │       └── chunk.ts             #   值对象：文本片段
│   │
...
│   ├── 项目运行.md                  # 运行指南
│   ├── 事件风暴.md                  # DDD 领域建模
│   ├── ask_step1.jpeg               # 系统案例截图（多轮问答 · 步骤 1）
│   ├── ask_setp2.jpeg               # 系统案例截图（步骤 2）
│   └── ask_setp3.jpeg               # 系统案例截图（步骤 3）
│
├── openspec/                        # OpenSpec 规格与变更
│   ├── specs/                       # 当前规格（6 个能力域，含 conversation-context）
│   └── changes/archive/             # 已归档变更（多轮迭代）
│
...
一次问答请求（HTTP POST body）至少包含**本轮问题**；可选携带 **会话 id**、是否 **流式**（SSE）。整体可理解为「先拿上下文 → 再检索 → 再作答 → 再落库」。下图使用 **Mermaid**（GitHub / GitLab / VS Code 等常见 Markdown 预览会渲染）。

```mermaid
flowchart TB
    subgraph g1["① 会话（可选）"]
        n1["POST：问题 · 可选会话 id · 可选 stream"]
        n2{带会话 id?}
        n3[历史为空]
        n4[读库已有消息]
        n5{有历史消息?}
...
**与单轮相比**：多轮时**检索**前多一步「把口语追问收成完整检索句」；**生成**时多带「历史」；**结束后**多写一条会话记录。无会话 id 时，①④ 可视为跳过，② 中检索句即用户原句，行为与早期单轮 RAG 一致。
...
- 架构上双存储分离、Provider 抽象、流式全链路打通，并支持多轮会话下的检索改写与历史裁剪，模块之间边界清晰，换模型或换向量库都不影响其他部分
