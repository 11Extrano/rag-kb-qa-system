...
一次问答请求（HTTP POST body）至少包含**本轮问题**；可选携带 **会话 id**、是否 **流式**（SSE）。整体可理解为「先拿上下文 → 再检索 → 再作答 → 再落库」。

```
客户端 POST
  body: 问题 + 可选「会话 id」+ 可选「stream: true」
         │
         ▼
┌─────────────────────────────────────────────────────────────┐
│ 1. 会话（可选）                                              │
│    若带「会话 id」→ 从持久化存储读出该会话下已有 user/assistant 消息 │
│    若无会话 id 或无记录 → 历史为空，等价单轮                 │
└─────────────────────────────────────────────────────────────┘
         │
         ▼
┌─────────────────────────────────────────────────────────────┐
│ 2. 检索                                                      │
│    有历史 → 用「历史 + 本轮问题」经一次大模型调用，得到「一句   │
│    仅用于检索的完整问句」；无历史 → 直接用本轮问题              │
│         ↓                                                    │
│    对「检索句」做嵌入 → 向量库 Top-K → 回关系库取片段原文与来源   │
│    （得到本轮【参考内容】列表，含相似度）                      │
└─────────────────────────────────────────────────────────────┘
         │
         ▼
┌─────────────────────────────────────────────────────────────┐
│ 3. 生成                                                      │
│    在 token 预算内组装：系统约束 + 裁剪后的多轮历史（若有）     │
│    + 【参考内容】+ 本轮问题；流式与一次性返回共用同一套组装与裁剪 │
│         │                                                    │
│         ├─ 非流式 → 一次返回完整答案 + 引用列表（JSON）         │
│         └─ 流式 → SSE 逐段推送 chunk，再推送 citations、结束    │
└─────────────────────────────────────────────────────────────┘
         │
         ▼
┌─────────────────────────────────────────────────────────────┐
│ 4. 会话落库（可选）                                          │
│    若带「会话 id」→ 将本轮用户问题与助手回复写入该会话          │
│    （供后续轮次拉取历史，不用于「会话列表」枚举）               │
└─────────────────────────────────────────────────────────────┘
         │
         ▼
    前端：非流式直接展示 JSON；流式则解析 SSE，拼字展示答案并展示引用
```

**与单轮相比**：多轮时**检索**前多一步「把口语追问收成完整检索句」；**生成**时多带「历史」；**结束后**多写一条会话记录。无会话 id 时，第 1、4 步跳过，第 2 步检索句即用户原句，行为与早期单轮 RAG 一致。
